...
import { use, getDirective, hasDirective } from '../src/plugin';
describe('Plugin System', () => {
    ...
    it('should execute a registered directive', () => {
        const mockPlugin = {
            name: 'test-plugin-2',
            install(api) {
                api.registerDirective('double', (value) => value * 2);
            },
        };
        use(mockPlugin);
    ...
    it('should allow multiple directives from one plugin', () => {
        const mockPlugin = {
            name: 'multi-plugin',
            install(api) {
                api.registerDirective('add', (a, b) => a + b);
                api.registerDirective('multiply', (a, b) => a * b);
            },
        };
        use(mockPlugin);
